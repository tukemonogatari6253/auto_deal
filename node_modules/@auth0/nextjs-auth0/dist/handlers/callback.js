    ...
    return function (req, res, session, state) {
        if (organization) {
            if (organization.startsWith('org_')) {
                ...
                (0, assert_1.strict)(session.user.org_name, 'Organization Name (org_name) claim must be a string present in the ID token');
                assert_1.strict.equal(session.user.org_name, organization.toLowerCase(), "Organization Name (org_name) claim value mismatch in the ID token; " +
                    "expected \"".concat(organization, "\", found \"").concat(session.user.org_name, "\""));
            }
        }
        if (afterCallback) {
